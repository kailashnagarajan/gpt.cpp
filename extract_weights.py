...
with open("weights/gpt2_weights.bin", "wb") as f:
    ...
    for i in range(n_head):
        ...
        write_tensor(f, sd[f"transformer.h.{i}.attn.c_attn.weight"])
        write_tensor(f, sd[f"transformer.h.{i}.attn.c_attn.bias"])
        write_tensor(f, sd[f"transformer.h.{i}.attn.c_proj.weight"])
        ...
        write_tensor(f, sd[f"transformer.h.{i}.mlp.c_fc.weight"])
        write_tensor(f, sd[f"transformer.h.{i}.mlp.c_fc.bias"])
        write_tensor(f, sd[f"transformer.h.{i}.mlp.c_proj.weight"])
